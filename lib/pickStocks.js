...
const VOL_RATIO_HARD_MAX = 3.5;

const RECENT_HIGH_LOOKBACK = 5;
const RECENT_RUNUP_3D_MAX = 0.08;
const MA_TOL = 0.995;

const BIAS_HARD_MAX = 0.12;
const RSI_HARD_MAX = 75;
...
function buildRecommendReason(s) {
  ...
  if (s.breakout) short.push("突破前高");
  if (s.ma5 >= s.ma20) short.push("趨勢轉強");
  if (s.volRatio && s.volRatio > 1.2) short.push("量能增溫");
  if (s.rsi14 && s.rsi14 >= 50 && s.rsi14 <= 70) short.push("動能健康");
  if (!s.hardRejected) short.push("未過熱");
  if (s.runup3dPct <= 8) short.push("起漲初段");

  let text = "技術面轉強，屬於可持續觀察的起漲型態。";

  if (s.breakout && s.volRatio > 1.2 && s.rsi14 <= 70 && s.runup3dPct <= 8) {
    text =
      "股價突破近5日前高，短線結構轉強，量能同步增溫且動能仍在合理區間，現階段仍屬起漲初段，可持續留意後續續航力。";
  } else if (s.breakout && s.ma5 >= s.ma20 && s.rsi14 <= 70) {
    text =
      "股價成功突破近階段壓力區，短中期均線結構轉強，且指標尚未進入過熱區，屬於較健康的轉強型態。";
  } else if (s.ma5 >= s.ma20 && s.volRatio > 1.2) {
    text =
      "短中期均線結構逐步翻揚，量能開始放大，顯示市場買盤有增溫跡象，屬於值得關注的轉強型標的。";
  } else if (s.rsi14 >= 50 && s.rsi14 <= 65 && s.runup3dPct <= 8) {
    text =
      "動能指標維持在健康區間，短線尚未明顯過熱，且近期漲幅仍屬可控範圍，較符合起漲初期的技術特徵。";
  } else if (s.breakout) {
    text =
      "股價已脫離短線整理區，出現突破跡象，若後續量能持續配合，則有機會延續目前的轉強走勢。";
  }
...
async function pickStocks({ generatedAt } = {}) {
  ...
  for (const p of pool) {
    try {
      ...
      const recommendReason = buildRecommendReason(s);
