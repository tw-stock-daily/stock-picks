...
const RECENT_RUNUP_3D_MAX = 0.08;   // 近3日漲超過8%直接淘汰
const MA_TOL = 0.995;

const BIAS_HARD_MAX = 0.12;         // close距MA20超過12%直接淘汰
const RSI_HARD_MAX = 75;            // RSI > 75直接淘汰
...
function buildRecommendReason(s) {
  ...
  if (s.ma5 >= s.ma20) short.push("站上MA20");
  if (s.volRatio && s.volRatio > 1.2) short.push("量能放大");
  if (s.rsi14 && s.rsi14 >= 50 && s.rsi14 <= 70) short.push("RSI健康");
  if (!s.hardRejected) short.push("未過熱");
  if (s.runup3dPct <= 8) short.push("尚未大漲");

  const textParts = [];
  if (s.breakout) textParts.push("股價突破近5日前高");
  if (s.ma5 >= s.ma20) textParts.push("站上MA20轉強");
  if (s.volRatio && s.volRatio > 1.2) textParts.push("量能放大");
  if (s.rsi14 && s.rsi14 <= 70) textParts.push("RSI仍在合理區間");
  if (s.runup3dPct <= 8) textParts.push("尚未大幅上漲");

  const text = textParts.length
    ? `${textParts.join("，")}，屬於起漲初期型態。`
    : "技術面轉強，屬於起漲初期型態。";
