...
const MIN_LIQ_SHARES = 600000;     // 股票池流動性門檻
const MIN_PRICE = 10;

const MIN_AVG_VOL20 = 800000;      // 20日均量至少 800 張
const VOL_RATIO_MIN = 1.15;        // 量能要明顯放大
const VOL_RATIO_STRONG = 1.60;
const VOL_RATIO_VERY_STRONG = 2.10;
...
function calcInstitutionScore(inst) {
  ...
  if ((inst.latestTrustNet ?? 0) > 50) score += 0.6;
  if ((inst.latestTotalNet ?? 0) < 0) score -= 0.8;
...
function buildReasonText({
  ...
  if (volRatio >= 2.1) parts.push("爆量");
  else if (volRatio >= 1.6) parts.push("量增");
...
function scoreStock(bars) {
  ...
  const i = c.length - 1;
  if (i < 35) return null;
  if (ma20Arr[i] == null || ma20Arr[i - 3] == null) return null;
  ...
  if (!(ma20 > ma20Arr[i - 3])) return null;

  // 量一定要成立
  if (volRatio < VOL_RATIO_MIN) return null;
  if (v[i] < avgVol20) return null;

  // MACD 至少不能翻空
  if (difNow < deaNow && histNow < 0) return null;
  ...
  if (ma20 > ma20Arr[i - 3]) score += 2.0;

  /* ===== 量能分（主體） ===== */
  score += (Math.min(volRatio, VOL_RATIO_CAP) - 1) * 5.4;
  if (volRatio >= VOL_RATIO_STRONG) score += 4.2;
  if (volRatio >= VOL_RATIO_VERY_STRONG) score += 5.8;
  if (v[i] > avgVol20 * 1.3) score += 2.0;

  /* ===== MACD 分 ===== */
  if (histTurnUp) score += 6.0;
  if (histNow > histPrev) score += 2.5;
  if (difAboveDea) score += 2.5;
  if (zeroAxisUp) score += 2.0;

  /* ===== 突破分 ===== */
  if (breakout20 && volRatio >= 1.3) score += 6.5;
  else if (breakout10 && volRatio >= 1.2) score += 4.0;
  else if (closeBreak10 && volRatio >= 1.2) score += 2.0;

  /* ===== K 棒強度 ===== */
  if (dayChg > 0.02) score += 2.2;
  if (close >= h[i] * 0.985) score += 2.0;
  if (lowerShadow > candleBody * 0.8 && dayChg > 0) score += 1.2;
  ...
  if (volRatio > 2.8 && upperShadow > Math.max(candleBody * 1.2, candleRange * 0.35)) score -= 3.0;
  if (dayChg < 0 && volRatio > 2.0) score -= 2.5;
  if (histNow < histPrev && histNow > 0) score -= 2.0;
  if ((breakout10 || breakout20) && close < open) score -= 2.5;
  if (upperShadow / candleRange > 0.45 && volRatio > 1.8) score -= 2.5;
