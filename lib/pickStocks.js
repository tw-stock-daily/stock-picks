 ...
 * - 修正 Yahoo chart 參數：range=6mo, interval=1d
 */
...
function buildReasonText({
  ...
  else if (breakout10 && volRatio >= 1.00) parts.push("10日轉強");
...
function scoreStock(bars) {
  ...
  /* ===== 量能分 ===== */
  if (volRatio >= VOL_RATIO_SOFT) score += 1.2;
  if (volRatio >= VOL_RATIO_STRONG) score += 3.8;
  if (volRatio >= VOL_RATIO_VERY_STRONG) score += 4.8;
  ...
  if (difNow > deaNow && histNow > histPrev) {
    score += 3.0;
  }
  ...
  if (runup3 > RECENT_RUNUP_SOFT) {
    score -= (runup3 - RECENT_RUNUP_SOFT) * 100 * 0.8;
  }
  ...
  if (volRatio > VOL_RATIO_EXTREME && upperShadow > Math.max(candleBody * 1.2, candleRange * 0.35)) score -= 2.8;
