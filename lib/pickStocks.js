...
const MIN_LIQ_SHARES = 800000;
const MIN_PRICE = 10;

const MIN_AVG_VOL20 = 1000000;
const VOL_RATIO_MIN = 1.10;
const VOL_RATIO_STRONG = 1.50;
const VOL_RATIO_VERY_STRONG = 2.00;
const VOL_RATIO_CAP = 5.0;
const VOL_RATIO_HARD_MAX = 8.0;

const BIAS_SOFT_MAX = 0.12;
const BIAS_HARD_MAX = 0.22;

const RECENT_RUNUP_SOFT = 0.12;
const RECENT_RUNUP_HARD = 0.25;
...
function calcInstitutionScore(inst) {
  ...
  if ((inst.latestTrustNet ?? 0) > 50) score += 1;
...
function buildReasonText({
  ...
  if (themeInfo?.matchedThemes?.length) {
    parts.push(`主流:${themeInfo.matchedThemes[0]}`);
  }

  if (breakout20 && volRatio >= 1.3) parts.push("20日突破");
  else if (breakout10 && volRatio >= 1.2) parts.push("10日轉強");

  if (volRatio >= 2.0) parts.push("爆量");
  else if (volRatio >= 1.5) parts.push("量增");
...
function scoreStock(bars) {
  ...
  if (difNow < deaNow && histNow < 0 && histPrev < 0) return null;

  let score = 0;

  score += (close / ma20 - 1) * 100 * 2.0;
  score += ma5 > ma20 ? 2.5 : 0;
  if (close > ma5) score += 1.5;

  score += (Math.min(volRatio, VOL_RATIO_CAP) - 1) * 4.8;
  if (volRatio >= VOL_RATIO_STRONG) score += 4;
  if (volRatio >= VOL_RATIO_VERY_STRONG) score += 6;

  if (histTurnUp) score += 7;
  if (histNow > histPrev) score += 2.5;
  if (difAboveDea) score += 2.5;
  if (zeroAxisUp) score += 1.5;

  if (breakout20 && volRatio >= 1.3) score += 7;
  else if (breakout10 && volRatio >= 1.25) score += 4;
  else if (closeBreak10 && volRatio >= 1.2) score += 2;

  if (dayChg > 0.02) score += 2.5;
  if (close >= h[i] * 0.985) score += 2;
  if (lowerShadow > candleBody * 0.8 && dayChg > 0) score += 1.5;

  if (runup3 > RECENT_RUNUP_SOFT) {
    score -= (runup3 - RECENT_RUNUP_SOFT) * 100 * 1.1;
  }
  if (bias > BIAS_SOFT_MAX) {
    score -= (bias - BIAS_SOFT_MAX) * 100 * 1.5;
  }
  if (runup5 > 0.12) score -= 4;
  if (bias > 0.12) score -= 4;

  if (volRatio > 2.5 && upperShadow > Math.max(candleBody * 1.2, candleRange * 0.35)) score -= 7;
  if (dayChg < 0 && volRatio > 2.0) score -= 5;
  if (histNow < histPrev && histNow > 0) score -= 3.5;
  if ((breakout10 || breakout20) && close < open) score -= 4.5;
  if (upperShadow / candleRange > 0.45 && volRatio > 1.8) score -= 4;
  if (rsiVal != null && rsiVal > 82) score -= 2.5;
...
async function processStock(s, hotThemes) {
  try {
    ...
    const finalScore = tech.score + instScore + (themeInfo.themeScore || 0);
