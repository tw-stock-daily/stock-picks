// lib/pickStocks.js
// ✅ 從你基準版 server.js 抽出的「選股核心」：不需要 express、不開 server
// ✅ 保留：600股票池 / RSI 50~82 / 兩段式FinMind加權TopK / 排除創新板創新版 / score>0 / TOP3主推+補位
// ✅ 供 GitHub Actions、App 端使用（產出 today.json）

const axios = require("axios");

// NodeCache 可選：若沒安裝也能跑（只是會更慢）
let NodeCache = null;
try { NodeCache = require("node-cache"); } catch (_) {}
const cache = NodeCache ? new NodeCache({ stdTTL: 60 * 15 }) : null;
...
// ============== cache helpers ==============
function cget(key) { return cache ? cache.get(key) : null; }
function cset(key, val, ttl) { if (cache) cache.set(key, val, ttl); }
function hasTokenTag(FINMIND_TOKEN) { return FINMIND_TOKEN ? "T" : "N"; }

// ============== FinMind stock info map (排除創新板) ==============
async function finmindFetch({ FINMIND_TOKEN, dataset, stock_id, data_id, start_date, end_date }) {
  const key = `finmind:${dataset}:${stock_id || ""}:${data_id || ""}:${start_date || ""}:${end_date || ""}:${hasTokenTag(FINMIND_TOKEN)}`;
...
async function finmindGetStockInfoMap({ FINMIND_TOKEN }) {
  const key = `finmind:TaiwanStockInfo:map:${hasTokenTag(FINMIND_TOKEN)}`;
  const cached = cget(key);
  if (cached) return cached;
  ...
  try {
    ...
    cset(key, map, 60 * 60 * 6);
    return map;
  } catch (_) {
    const empty = new Map();
    cset(key, empty, 60 * 10);
    return empty;
  }
}
...
async function pickStocks(options = {}) {
  ...
  return {
    ...
    pool: { type: "TWSE_TOP_VOLUME", size: symbols.length, POOL_SIZE, MIN_LIQ_SHARES, MIN_PRICE },
