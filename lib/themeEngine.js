...
const THEME_KEYWORDS = {
  "AI伺服器": ["ai伺服器","伺服器","gpu","資料中心","csp","機櫃","b300","gb200","ai server"],
  "CPO/光通訊": ["cpo","光通訊","矽光子","800g","1.6t","高速傳輸","光模組","交換器"],
  "半導體": ["半導體","晶圓代工","先進製程","封測","先進封裝","cowos","hbm","ic設計","晶片"],
  "PCB/載板": ["pcb","載板","abf","銅箔基板","高階板"],
  "散熱": ["散熱","液冷","均熱板","熱管","風扇"],
  "電源/連接器": ["電源","power","連接器","線束","伺服器電源","功率元件"],
  "機器人/自動化": ["機器人","自動化","智慧工廠","工業電腦","人形機器人","伺服系統"],
  "車用電子": ["車用","電動車","adas","robotaxi","自駕","車載","充電樁"],
  "重電/儲能": ["重電","儲能","電網","變壓器","綠能","電力設備"],
  "軍工/航太": ["軍工","航太","無人機","國防","飛機","軍備"]
};
...
function calcThemeScoreForStock(stockId, hotThemes) {
  ...
  for (const ht of hotThemes || []) {
    ...
    if (roots.includes(themeName) || tags.includes(themeName)) {
      score += Math.min(INDUSTRY_BASE_BONUS[themeName] || 2, hotScore * 0.45);
    ...
    if (linked) {
      score += Math.min((INDUSTRY_BASE_BONUS[themeName] || 2) * 0.45, hotScore * 0.22);
      matchedThemes.push(themeName);
